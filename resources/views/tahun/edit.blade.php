...
@section('content')
    <div class="intro-y flex flex-col sm:flex-row items-center mt-8">
        <h2 class="text-lg font-medium mr-auto">
          Form Edit Data Tahun Keputusan
        </h2>
    </div>
    <div class="intro-y datatable-wrapper box p-5 mt-5">
        <form class="validate-form" action="/tahun/{{ $tahun->id }}" method="POST">
            ...
            <div class="mb-3">
                ...
                <div>
                    <label class="flex flex-col sm:flex-row"> Keterangan <span class="sm:ml-auto mt-1 sm:mt-0 text-xs text-gray-600">Wajib Diisi</span> </label> <input type="text" name="keterangan" class="input w-full border mt-2  @error('keterangan') error @enderror()" required value="{{ $tahun->keterangan }}">
                    @error('keterangan')
                    <label id="name-error" class="error" for="name">{{ $message }}</label>
                    @enderror 
                   </div>
                <button type="submit" class="button text-white bg-theme-1 shadow-md mr-2 mt-3">Update</button>
        </form>
    </div>
