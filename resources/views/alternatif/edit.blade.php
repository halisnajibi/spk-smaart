...
@section('content')
    ...
    <div class="intro-y datatable-wrapper box p-5 mt-5">
        <form class="validate-form" novalidate="novalidate"  method="POST" action="/alternatif/{{ $alternatif->id }}">
            @method('put')
            @csrf
            ...
            <div class="mb-2">
                <select class="select2 w-full" name="tahun_id">
                    ...
                    <option value="{{ $tahun->id }}" {{ ($alternatif->tahun_id == $tahun->id) ? 'selected':'' }}>{{ $tahun->kode }}-{{ $tahun->tahun }}</option>
                    @endforeach
                </select>
            <div class="mb-3">
                ...
                <input type="text" name="judul" class="input w-full border mt-2 @error('judul') error @enderror()" placeholder="Judul"
                    minlength="2" value="{{ $alternatif->judul }}">
                    @error('judul')
            ...
            <div class="mb-3">
                ...
                <input type="text" name="keterangan" class="input w-full border mt-2 @error('keterangan') error @enderror()" placeholder="Keterangan"
                    minlength="2" value="{{ $alternatif->keterangan }}">
                    @error('keterangan')
                    <label id="name-error" class="error" for="name">{{ $message }}</label>
                    @enderror
            </div>
